...
function submit_it()   
     {
		...
		if(AppForm.jumin1.value.length<1){
			alert("주민번호 입력을 확인바랍니다");
			AppForm.jumin1.focus();
			return;
			}
     	
		if(AppForm.jumin2.value.length<1){
			alert("주민번호 입력을 확인바랍니다");
			AppForm.jumin2.focus();
			return;
			}
     	//우편번호
...
function cngSelectDay(value) {
	iSelectDay.location = "register_core_selectcourse.asp?courseid=" + value;
}

function chkApplication(value1, value2) {
	iSelectDay.location = "register_core_check.asp?courseid=" + value1 + "&d=" + value2 + "&studid=<%=Studid%>";
}
...
function cancel()
{
	alert("핵심과정 지원을 취소하셨습니다.")
	location.href="/register/register_apply.asp"
}
...
{
	window.open("zipserch.asp?tozip=C&path=comp","","height=180,width=400,status=no,toolbar=no,titlebar=no,menubar=no,location=0,left=300,top=300")	
}
...
<script language="vbscript">
	sub zipsearch_onClick()	//'우편번호체크
		window.open "zipserch.asp?tozip=C",null,"height=180,width=400,status=no,toolbar=no,titlebar=no,menubar=no,location=0,left=300,top=300"
	end sub                                                 
</script>